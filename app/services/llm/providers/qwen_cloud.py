...
def _extract_content(response: Any) -> str:
    ...
    if not choices:
        logger.warning("qwen_cloud_completion_empty_choices")
        return ""
...
class QwenCloudProvider(LLMProvider):
    def __init__(
        ...
        self._api_key = api_key or settings.text_model.qwen_api_key
        self._api_base = api_base or settings.text_model.qwen_api_base
        self._default_model = default_model or settings.text_model.qwen_model_name
    ...
    async def generate(self, request: LLMRequest) -> LLMResponse:
        ...
        try:
            ...
            elapsed = time.monotonic() - start
            logger.error(
                "qwen_cloud_generate_failed",
                model=model_id,
                elapsed_ms=round(elapsed * 1000, 2),
                error=str(exc),
                error_type=type(exc).__name__,
            )
            raise AppError(ErrorCode.CLOUD_MODEL_ERROR, "云API调用失败") from exc
