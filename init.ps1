...
Write-Host ""
Write-Host "[1/6] Checking Python environment..."
...
Write-Host ""
Write-Host "[2/6] Checking configuration..."
...
Write-Host ""
Write-Host "[3/6] Checking feature list..."
...
if (Test-Path "tests") {
    ...
    else { Write-Host "  WARN: Some tests failing" }
} else {
    Write-Host "  INFO: No tests directory yet"
}

Write-Host ""
Write-Host "[5/6] Checking FACT_REGISTRY consistency..."
...
Write-Host ""
Write-Host "[6/6] Generating system-state.json..."
Invoke-Expression "$python scripts/update_system_state.py"

Write-Host ""
Write-Host "=== Init Complete (6 steps) ==="
