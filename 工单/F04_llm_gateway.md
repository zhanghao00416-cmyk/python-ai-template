...
## 功能三元组
- **行为**：LLM 调用统一经 gateway，支持 qwen_cloud/vllm 双通道切换，stream/non-stream 统一方法
- **验证**：`pytest tests/test_04_llm_gateway.py`
- **状态**：not_started → active → passing

## 验收标准（全部通过才能标 passing）
- [ ] `pytest tests/test_04_llm_gateway.py` 通过
...
- F03: `app/core/errors.py`, `app/middleware/*`
...
```text
按 AGENTS.md §0.1 启动；执行工单 F04，严格阶段 1→2→3。
阶段 1：读 DEPENDENCY_MAP F04 行 + 上文 docs + 前序代码，首段约束摘要 ≥5 条，输出 §1.3 交付物，禁止写代码。
阶段 2：仅实现 §2.2 清单；验证：pytest tests/test_04_llm_gateway.py
