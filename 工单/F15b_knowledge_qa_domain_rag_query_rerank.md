# 工单 F15b：知识库RAG查询（4种召回策略+rerank+父子上下文返回）
...
## 功能三元组
- **行为**：RAG查询（4种召回+rerank+父子上下文）→SSE/同步双模式→citation+retrieval_results返回
- **验证**：`pytest tests/test_15b_knowledge_rag.py`
- **状态**：not_started → active → passing

## 验收标准（全部通过才能标 passing）
- [ ] `pytest tests/test_15b_knowledge_rag.py` 通过
...
- F15a: `app/domain/knowledge/service.py`
...
- `docs/00-meta/DEPENDENCY_MAP.md`（本工单 F15b 行）
...
```text
按 AGENTS.md §0.1 启动；执行工单 F15b，严格阶段 1→2→3。
阶段 1：读 DEPENDENCY_MAP F15b 行 + 上文 docs + 前序代码，首段约束摘要 ≥5 条，输出 §1.3 交付物，禁止写代码。
阶段 2：仅实现 §2.2 清单；验证：pytest tests/test_15b_knowledge_rag.py
