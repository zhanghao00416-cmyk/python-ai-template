# 工单 F12：多 Agent 协作（Orchestrator 模式）
...
## 功能三元组
- **行为**：Orchestrator 分配 3 个子 agent 任务 → 汇总结果，轨迹记录完整
- **验证**：`pytest tests/test_12_multi_agent.py`
- **状态**：not_started → active → passing

## 验收标准（全部通过才能标 passing）
- [ ] `pytest tests/test_12_multi_agent.py` 通过
...
- F11: `app/agent/base.py`, `app/agent/state.py`, `app/agent/react.py`

### 1.2 必读 docs（DEPENDENCY_MAP 表 2 + 工单列表）

- `docs/02-engineering/AGENT_SPEC.md`
- `docs/00-meta/DEPENDENCY_MAP.md`（本工单 F12 行）
...
```text
按 AGENTS.md §0.1 启动；执行工单 F12，严格阶段 1→2→3。
阶段 1：读 DEPENDENCY_MAP F12 行 + 上文 docs + 前序代码，首段约束摘要 ≥5 条，输出 §1.3 交付物，禁止写代码。
阶段 2：仅实现 §2.2 清单；验证：pytest tests/test_12_multi_agent.py
