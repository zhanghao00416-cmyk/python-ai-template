# 工单 F01：项目骨架 + FastAPI 入口 + DI 容器 + YAML 配置
...
## 功能三元组
- **行为**：FastAPI 启动，/health 返回 200，DI 容器可用，YAML 配置加载成功
- **验证**：`curl http://localhost:6006/api/v1/health && pytest tests/test_01_skeleton.py`
- **状态**：not_started → active → passing

## 验收标准（全部通过才能标 passing）
- [ ] `curl http://localhost:6006/api/v1/health && pytest tests/test_01_skeleton.py` 通过
...
- 无跨工单硬代码依赖（仍须读 DEPENDENCY_MAP 与本工单 docs）
...
- `docs/00-meta/DEPENDENCY_MAP.md`（本工单 F01 行）
...
```text
按 AGENTS.md §0.1 启动；执行工单 F01，严格阶段 1→2→3。
阶段 1：读 DEPENDENCY_MAP F01 行 + 上文 docs + 前序代码，首段约束摘要 ≥5 条，输出 §1.3 交付物，禁止写代码。
阶段 2：仅实现 §2.2 清单；验证：curl http://localhost:6006/api/v1/health && pytest tests/test_01_skeleton.py
