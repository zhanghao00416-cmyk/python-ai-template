...
## 功能三元组
- **行为**：查询/修改 prompt 文件，版本历史记录，回滚指定版本，重置基准副本，模板变量替换，启动时预加载
- **验证**：`pytest tests/test_08_prompt_manager.py`
- **状态**：not_started → active → passing

## 验收标准（全部通过才能标 passing）
- [ ] `pytest tests/test_08_prompt_manager.py` 通过
...
```text
按 AGENTS.md §0.1 启动；执行工单 F08，严格阶段 1→2→3。
阶段 1：读 DEPENDENCY_MAP F08 行 + 上文 docs + 前序代码，首段约束摘要 ≥5 条，输出 §1.3 交付物，禁止写代码。
阶段 2：仅实现 §2.2 清单；验证：pytest tests/test_08_prompt_manager.py
