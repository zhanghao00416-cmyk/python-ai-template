...
## 功能三元组
- **行为**：错误请求返回结构化 JSON（含 code/message/request_id），trace_id 贯穿请求
- **验证**：`pytest tests/test_03_error_middleware.py`
- **状态**：not_started → active → passing

## 验收标准（全部通过才能标 passing）
- [ ] `pytest tests/test_03_error_middleware.py` 通过
...
- F01: `app/core/errors.py`, `app/core/response.py`, `app/core/context.py`

### 1.2 必读 docs（DEPENDENCY_MAP 表 2 + 工单列表）

- `docs/01-architecture/ERROR_CODE.md`
- `docs/00-meta/DEPENDENCY_MAP.md`（本工单 F03 行）
...
```text
按 AGENTS.md §0.1 启动；执行工单 F03，严格阶段 1→2→3。
阶段 1：读 DEPENDENCY_MAP F03 行 + 上文 docs + 前序代码，首段约束摘要 ≥5 条，输出 §1.3 交付物，禁止写代码。
阶段 2：仅实现 §2.2 清单；验证：pytest tests/test_03_error_middleware.py
