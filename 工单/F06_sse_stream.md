...
## 功能三元组
- **行为**：SSE 端点推送完整事件流（start/intent/chunk/citation/heartbeat/progress/usage/done/error）
- **验证**：`pytest tests/test_06_sse_stream.py`
- **状态**：not_started → active → passing

## 验收标准（全部通过才能标 passing）
- [ ] `pytest tests/test_06_sse_stream.py` 通过
...
- F03: `app/core/response.py`, `app/core/constants.py`

### 1.2 必读 docs（DEPENDENCY_MAP 表 2 + 工单列表）

- `docs/01-architecture/API_CONTRACT.md`
- `docs/00-meta/DEPENDENCY_MAP.md`（本工单 F06 行）
...
```text
按 AGENTS.md §0.1 启动；执行工单 F06，严格阶段 1→2→3。
阶段 1：读 DEPENDENCY_MAP F06 行 + 上文 docs + 前序代码，首段约束摘要 ≥5 条，输出 §1.3 交付物，禁止写代码。
阶段 2：仅实现 §2.2 清单；验证：pytest tests/test_06_sse_stream.py
