# 工单 F15a：知识库集合+文档管理（4种分块+父子模式+3元数据+清库确认）
...
## 功能三元组
- **行为**：创建集合→上传文档（4种分块+父子分块模式+3元数据+异步）→多维过滤查看→条件删除+一键清库
- **验证**：`pytest tests/test_15a_knowledge_docs.py`
- **状态**：not_started → active → passing

## 验收标准（全部通过才能标 passing）
- [ ] `pytest tests/test_15a_knowledge_docs.py` 通过
...
- F05: `app/infra/vector_store.py`
- F08: `app/services/prompt_manager.py`

### 1.2 必读 docs（DEPENDENCY_MAP 表 2 + 工单列表）

- `docs/04-kb/QDRANT_COLLECTION_CONFIG.md`
- `docs/03-ai/RAG_PIPELINE_SPEC.md`
- `docs/00-meta/DEPENDENCY_MAP.md`（本工单 F15a 行）
...
```text
按 AGENTS.md §0.1 启动；执行工单 F15a，严格阶段 1→2→3。
阶段 1：读 DEPENDENCY_MAP F15a 行 + 上文 docs + 前序代码，首段约束摘要 ≥5 条，输出 §1.3 交付物，禁止写代码。
阶段 2：仅实现 §2.2 清单；验证：pytest tests/test_15a_knowledge_docs.py
