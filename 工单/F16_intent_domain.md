...
## 功能三元组
- **行为**：POST /api/v1/intent 三层漏斗（关键字→相似度→LLM），多意图检测+query重组，返回主意图+子意图
- **验证**：`pytest tests/test_16_intent.py`
- **状态**：not_started → active → passing

## 验收标准（全部通过才能标 passing）
- [ ] `pytest tests/test_16_intent.py` 通过
...
```text
按 AGENTS.md §0.1 启动；执行工单 F16，严格阶段 1→2→3。
阶段 1：读 DEPENDENCY_MAP F16 行 + 上文 docs + 前序代码，首段约束摘要 ≥5 条，输出 §1.3 交付物，禁止写代码。
阶段 2：仅实现 §2.2 清单；验证：pytest tests/test_16_intent.py
