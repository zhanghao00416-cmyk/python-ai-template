# 工单 F18：可观测性体系（Tracing + Token 统计 + 指标）
...
## 功能三元组
- **行为**：执行请求→日志含 trace_id+token_usage+latency，/metrics 暴露 Prometheus 指标（LLM/HTTP/KB/Agent 10项指标）
- **验证**：`pytest tests/test_18_observability.py && curl /metrics`
- **状态**：not_started → active → passing

## 验收标准（全部通过才能标 passing）
- [ ] `pytest tests/test_18_observability.py && curl /metrics` 通过
...
- `ARCHITECTURE.md`
- `docs/00-meta/DEPENDENCY_MAP.md`（本工单 F18 行）
...
```text
按 AGENTS.md §0.1 启动；执行工单 F18，严格阶段 1→2→3。
阶段 1：读 DEPENDENCY_MAP F18 行 + 上文 docs + 前序代码，首段约束摘要 ≥5 条，输出 §1.3 交付物，禁止写代码。
阶段 2：仅实现 §2.2 清单；验证：pytest tests/test_18_observability.py && curl /metrics
