# 工单 F14：Chat 对话域（自动创建会话+SSE/同步双模式）
...
## 功能三元组
- **行为**：POST /api/v1/chat SSE 流完整（start→chunk→usage→done），自动创建会话，支持同步模式，上下文窗口自动截取
- **验证**：`pytest tests/test_14_chat.py && curl SSE 端点完整流`
- **状态**：not_started → active → passing

## 验收标准（全部通过才能标 passing）
- [ ] `pytest tests/test_14_chat.py && curl SSE 端点完整流` 通过
...
- `docs/00-meta/DEPENDENCY_MAP.md`（本工单 F14 行）
...
```text
按 AGENTS.md §0.1 启动；执行工单 F14，严格阶段 1→2→3。
阶段 1：读 DEPENDENCY_MAP F14 行 + 上文 docs + 前序代码，首段约束摘要 ≥5 条，输出 §1.3 交付物，禁止写代码。
阶段 2：仅实现 §2.2 清单；验证：pytest tests/test_14_chat.py && curl SSE 端点完整流
