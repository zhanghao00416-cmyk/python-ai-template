# 工单 F10：Tools 注册中心 + MCP 适配器 + Skills
...
## 功能三元组
- **行为**：注册 tool/skill → 通过 registry 调用 → MCP 协议返回结果 → skill 遮蔽 tool 互斥
- **验证**：`pytest tests/test_10_tools_registry.py`
- **状态**：not_started → active → passing

## 验收标准（全部通过才能标 passing）
- [ ] `pytest tests/test_10_tools_registry.py` 通过
...
- `docs/00-meta/DEPENDENCY_MAP.md`（本工单 F10 行）
...
```text
按 AGENTS.md §0.1 启动；执行工单 F10，严格阶段 1→2→3。
阶段 1：读 DEPENDENCY_MAP F10 行 + 上文 docs + 前序代码，首段约束摘要 ≥5 条，输出 §1.3 交付物，禁止写代码。
阶段 2：仅实现 §2.2 清单；验证：pytest tests/test_10_tools_registry.py
