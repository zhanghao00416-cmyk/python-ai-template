...
## 功能三元组
- **行为**：docker-compose up → 全 API 可调通 → feature_list.json 全部 passing
- **验证**：`docker-compose up -d && curl 全端点 && bash init.sh`
- **状态**：not_started → active → passing

## 验收标准（全部通过才能标 passing）
- [ ] `docker-compose up -d && curl 全端点 && bash init.sh` 通过
...
- F01–F14, F15a, F15b, F15c, F16–F20 全部前序代码

### 1.2 必读 docs（DEPENDENCY_MAP 表 2 + 工单列表）

- `docs/02-engineering/DEPLOYMENT_GUIDE.md`
- `docs/00-meta/DEPENDENCY_MAP.md`（本工单 F21 行）
...
```text
按 AGENTS.md §0.1 启动；执行工单 F21，严格阶段 1→2→3。
阶段 1：读 DEPENDENCY_MAP F21 行 + 上文 docs + 前序代码，首段约束摘要 ≥5 条，输出 §1.3 交付物，禁止写代码。
阶段 2：仅实现 §2.2 清单；验证：docker-compose up -d && curl 全端点 && bash init.sh
