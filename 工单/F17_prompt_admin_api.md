# 工单 F17：Prompt 管理 API（查询+修改+版本历史+重置）
...
## 功能三元组
- **行为**：查询模板（分页过滤+详情），修改模板（自动版本+回滚），版本历史列表，重置基准版本
- **验证**：`pytest tests/test_17_prompt_admin.py`
- **状态**：not_started → active → passing

## 验收标准（全部通过才能标 passing）
- [ ] `pytest tests/test_17_prompt_admin.py` 通过
...
- `docs/00-meta/DEPENDENCY_MAP.md`（本工单 F17 行）
...
```text
按 AGENTS.md §0.1 启动；执行工单 F17，严格阶段 1→2→3。
阶段 1：读 DEPENDENCY_MAP F17 行 + 上文 docs + 前序代码，首段约束摘要 ≥5 条，输出 §1.3 交付物，禁止写代码。
阶段 2：仅实现 §2.2 清单；验证：pytest tests/test_17_prompt_admin.py
