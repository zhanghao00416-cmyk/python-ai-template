...
## 功能三元组
- **行为**：创建集合→上传文档→RAG查询→删除文档/集合 全流程端到端验证
- **验证**：`pytest tests/test_15c_knowledge_e2e.py && curl 全流程`
- **状态**：not_started → active → passing

## 验收标准（全部通过才能标 passing）
- [ ] `pytest tests/test_15c_knowledge_e2e.py && curl 全流程` 通过
...
```text
按 AGENTS.md §0.1 启动；执行工单 F15c，严格阶段 1→2→3。
阶段 1：读 DEPENDENCY_MAP F15c 行 + 上文 docs + 前序代码，首段约束摘要 ≥5 条，输出 §1.3 交付物，禁止写代码。
阶段 2：仅实现 §2.2 清单；验证：pytest tests/test_15c_knowledge_e2e.py && curl 全流程
